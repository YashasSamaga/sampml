    ...
    public:
        ...
        void submit(const input_vector& raw_input) {
            ...
            */
            ...
            LOCLOSAngleDifferences.push_back(std::abs(std::acos(angle)) * 180/pi());
            ...
            */
            ...
            if(shots > features::SHOTS_PER_SAMPLE) {
                ...
                for(int i = 0; i < HitVictimVelocityPerpDeltas_dm.size(); i++)
                    HitVictimVelocityPerpDeltas_dm(i) = std::abs(HitVictimVelocityPerpDeltas_dm(i));     
                ...
                for(int i = 0; i < ImmediateMissVictimVelocityPerpDeltas_dm.size(); i++)
                    ImmediateMissVictimVelocityPerpDeltas_dm(i) = std::abs(ImmediateMissVictimVelocityPerpDeltas_dm(i));     
                ...
                for(int i = 0; i < ShooterHorizontalAngleRates_dm.size(); i++) {
                    int ang = std::abs(ShooterHorizontalAngles[i + 1] - ShooterHorizontalAngles[i]);
                    ShooterHorizontalAngleRates_dm(i) = std::min(ang, 360 - ang);
                }
