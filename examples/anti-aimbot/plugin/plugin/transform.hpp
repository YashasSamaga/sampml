        ...
        void submit(const input_vector& input) {
           ...
           // || input(iVictimSpecialAction) != samp::SPECIAL_ACTION_NONE
            ...
            || input(bVictimSurfingObject)) { /* not M4 */
                logprintf("REJECTED 1 %d %d %d %d", input(iShooterWeaponID), input(bShooterSurfingVehicle), input(bShooterSurfingObject), input(bVictimSurfingVehicle), input(bVictimSurfingObject));
                reset();
                return;
            }
